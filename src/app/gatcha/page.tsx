...
export default function GatchaSimulatorPage() {
  ...
  const [roledCards, setRoledCards] = useState<
    Array<MACard & { number: number }> | any
  >([])
  ...
  const roleCards = () => {
    ...
    const selectedCards = new Array(roledAmount)
      .fill(1)
      .map(() => getRandomCard(title))
  ...
  return (
    <div className={'w-full mt-[15px]'}>
      ...
      <div className={'text-start mb-[10px]'}>
        * 아래 픽업 목록중 하나를 클릭하여 선택 후 공명하세요. (현재 1개 밖에
        없어서 극지의 여정 클릭)
      </div>
      <div className={'flex'}>
        {gatchaTitles.map((gatchaTitle) => {
          return (
            <div
              key={createKey()}
              className={
                'cursor-pointer rounded overflow-hidden border-2 border-gray-400 flex items-center justify-center shadow-md shadow-gray-800'
              }
              onClick={() => {
        ...
        <div className={'text-start mt-[10px]'}>
          ...
          <div className={'flex flex-col items-start gap-[10px]'}>
            <div className={'flex gap-[10px] items-center'}>
              <div
                className={
                  'select-none cursor-pointer bg-gray-800 text-white px-[10px] py-[4px] rounded-xl shadow-md shadow-gray-800 border-white border'
                }
                onClick={() => roleCards()}
              >
                공명 10회
              ...
              <div>
                ...
                <div
                  className={
                    'flex gap-[4px] [&>div]:border [&>div]:border-gray-800 [&>div]:px-[4px] [&>div]:rounded'
                  }
                >
                  <div>
                    【MR】
          ...
          {title.list.map((list) => {
            return (
              <div
                key={createKey()}
                className={'mb-[10px] border-b border-gray-600 border-dashed'}
              >
